...
export const Category: React.FC = () => {
  ...
  const headers = {
    'ai-data': {
      ...
      icon: Brain,
      color: 'from-red-950/40 via-[#050505] to-[#050505]'
    },
  ...
  if (!isValidCategory) {
    return (
      <div className="min-h-screen bg-[#050505] text-white flex flex-col items-center justify-center p-6 text-center">
        <h1 className="font-montserrat font-black text-4xl text-white mb-4">CATEGORY NOT FOUND</h1>
        <p className="font-montserrat text-sm text-white/60 mb-8">The requested discipline category does not exist.</p>
        <Link
          to="/"
  ...
  return (
    <div className="w-full bg-[#050505] text-white min-h-screen pt-28 sm:pt-36 pb-24">
      {/* Category Hero Header */}
      <section className="relative w-full border-b border-white/10 px-6 sm:px-10 lg:px-16 pb-16 lg:pb-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10 space-y-8">
          ...
          <div>
            <button
              onClick={() => navigate('/')}
              className="inline-flex items-center gap-2 text-xs font-montserrat font-semibold tracking-widest text-white/60 hover:text-white uppercase transition-colors group border border-white/10 hover:border-red-500/50 bg-[#0A0A0A] px-4 py-2 cursor-pointer"
            >
          ...
          <div className="space-y-4 max-w-4xl">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-red-950/60 border border-red-500/40 text-red-400 rounded">
                <CategoryIcon className="w-5 h-5" />
              </div>
              <span className="font-montserrat font-extrabold text-xs text-red-500 tracking-[0.25em] uppercase">
                {projects.length} REPOSITORIES
              </span>
            </div>

            <h1 className="font-montserrat font-black uppercase text-4xl sm:text-5xl lg:text-6xl text-white tracking-tight leading-none">
              {currentHeader.title}
            </h1>

            <p className="font-montserrat font-bold text-xs sm:text-sm text-red-400 uppercase tracking-widest">
              {currentHeader.subtitle}
            </p>

            <p className="font-montserrat text-sm sm:text-base text-white/70 leading-relaxed max-w-2xl pt-2">
